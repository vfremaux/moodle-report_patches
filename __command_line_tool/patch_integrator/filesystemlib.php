...
* @param string $path the relative path from baseroot
...
function filesystem_create_dir($path, $recursive = 0, $pathbase=null) {
    ...
    $result = true;
    if (!$recursive){
       if (@$CFG->filedebug) report('debug', "creating dir <i>{$path}</i><br/>");
       $oldMask = umask(0);
       ...
       return $result;
        ...
        $parts = explode('/', $path);
        if (preg_match("/:$/", $parts[0])){ // resolves Windows type pathes
            $pathTo = $parts[0];
            $startindex = 1;
        } else {
            $pathTo = '';
        ...
        for($i = $startindex; $i < count($parts) && $result; $i++){
            $pathTo .= '/' . $parts[$i];
            $result = $result && filesystem_create_dir($pathTo, 0, $pathbase);
        }
        return $result;
    }
}
...
* @param string $relativepath the path from baseroot
* @param string $pathbase the base path
*/
function filesystem_is_dir($relativepath, $pathbase=null){
    ...
    if (@$CFG->filedebug) report('debug', "is dir <i>$pathbase$relativepath</i><br/>");
...
function filesystem_file_exists($relativepath, $pathbase=null){
    ...
    if (@$CFG->filedebug) report('debug', "file exists <i>$pathbase$relativepath</i><br/>");
...
function filesystem_scan_dir($relativepath, $hiddens = 0, $what = 0, $pathbase=null){
    ...
    if (@$CFG->filedebug) report('debug', "scanning <i>$pathbase$relativepath</i><br/>");
...
function filesystem_clear_dir($relativepath, $fullDelete = false, $pathbase=null) {
    ...
    if (@$CFG->filedebug) report('debug', "clearing dir <i>$pathbase$relativepath</i><br/>");
...
* @param string $source the source path from baseroot
* @param string $dest the dest path from baseroot
* @param string $pathbase the base path
* @return void
*/
function filesystem_copy_tree($source, $dest, $pathbase=null) {
   ...
   if (@$CFG->filedebug) report('debug', "copying tree <i>$pathbase$source</i> to <i>$pathbase$dest</i><br/>");
...
function filesystem_store_file($relativepath, $data, $pathbase=null) {
    ...
    if (@$CFG->filedebug) report('debug', "storing <i>$pathbase$relativepath</i><br/>");
    $parts = pathinfo($relativepath);
    if (!filesystem_is_dir($parts['dirname'], $pathbase)) filesystem_create_dir($parts['dirname'], FS_RECURSIVE, $pathbase);
...
function filesystem_read_a_file($relativepath, $pathbase=null) {
    ...
    if (@$CFG->filedebug) report('debug', "reading <i>$pathbase$relativepath</i><br/>");
...
function filesystem_delete_file($relativepath, $pathbase=null){
    ...
    if (@$CFG->filedebug) report('debug', "deleting file <i>$relativepath</i><br/>");
...
function filesystem_remove_dir($relativepath, $pathbase=null){
    ...
    if (@$CFG->filedebug) report('debug', "deleting dir <i>$relativepath</i><br/>");
...
function filesystem_move_file($source, $dest, $pathbase=null){
    ...
    if (filesystem_file_exists($source, $pathbase)){
        if (@$CFG->filedebug) report('debug', "moving file/dir <i>$source</i> to <i>$dest</i><br/>");
...
function filesystem_copy_file($source, $dest, $pathbase=null) {
    ...
    if (@$CFG->filedebug) report('debug', "copying file <i>$pathbase$source</i> to <i>$pathbase$dest</i><br/>");
...
* @param string $path the path from baseroot
